...
SPLITS = {
    ...
    "mbpp": ["full", "sanitized"],
    "MBPP": ["base", "plus"],
    "SimulatedHumanEval": ["base", "plus"],
    "SimulatedMbpp": ["full", "sanitized"],
    "SimulatedMBPP": ["base", "plus"]
}
...
def check_args(args):
    ...
    # Check for simulated_data_path when task is any of the simulated versions
    if args.task.startswith("Simulated") and not args.simulated_data_path:
        logger.error(f"For {args.task}, --simulated_data_path must be provided.")
...
def get_args(parser):
    ...
    #===================Simulated Study Parser===================
    parser.add_argument("--simulated_data_path", default=None, type=str, help="Path to the simulated user study data file.")
